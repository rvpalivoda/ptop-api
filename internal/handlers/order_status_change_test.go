...
func TestOrderStatusChangeFlow(t *testing.T) {
    ...
    if err := db.Create(&asset1).Error; err != nil { t.Fatalf("asset: %v", err) }
    if err := db.Create(&asset2).Error; err != nil { t.Fatalf("asset: %v", err) }
    offer := models.Offer{
    ...
    if err != nil || resp.StatusCode != http.StatusSwitchingProtocols { t.Fatalf("buyer ws: %v %d", err, resp.StatusCode) }
    ...
    if err != nil || resp2.StatusCode != http.StatusSwitchingProtocols { t.Fatalf("seller ws: %v %d", err, resp2.StatusCode) }
    ...
    if err := connBuyer.ReadJSON(&sevt); err != nil { t.Fatalf("buyer status evt: %v", err) }
    if sevt.Type != "order.status_changed" || sevt.Order.Status != models.OrderStatusPaid { t.Fatalf("unexpected paid evt buyer: %#v", sevt) }
    if err := connSeller.ReadJSON(&sevt); err != nil { t.Fatalf("seller status evt: %v", err) }
    if sevt.Order.Status != models.OrderStatusPaid { t.Fatalf("unexpected paid evt seller: %#v", sevt) }
    ...
    r.ServeHTTP(w, req)
    if w.Code != http.StatusOK { t.Fatalf("release %d", w.Code) }
    if err := connBuyer.ReadJSON(&sevt); err != nil { t.Fatalf("buyer release evt: %v", err) }
    if sevt.Order.Status != models.OrderStatusReleased { t.Fatalf("unexpected release evt buyer: %#v", sevt) }
    if err := connSeller.ReadJSON(&sevt); err != nil { t.Fatalf("seller release evt: %v", err) }
    if sevt.Order.Status != models.OrderStatusReleased { t.Fatalf("unexpected release evt seller: %#v", sevt) }
}
